...
const Box:React.FC<props> = ({setDetails}) => {
    const [hovered, sethovered] = useState(false)
    return ( 
        <div className=" w-full h-[105vw] sm:w-[45vw] sm:h-[45vh] md:h-[55vh] lg:h-[30vw] lg:w-[26vw] xl:w-[28vw] relative cursor-pointer z-10 "  
        onMouseEnter={()=> {sethovered(true); console.log("hover",hovered)   }}
        onMouseLeave={()=> {sethovered(false); console.log(hovered)   }}
        onClick={()=>{setDetails(true)}}
        >   
            <Image id='hovered' alt="Project"  src={me} className="w-full h-full opacity-75 hover:opacity-50 duration-700 "  ></Image>
            {hovered && 
            <>
            <div className={`bg-white shadow-2xl absolute bottom-5 w-[90%] right-[50%] translate-x-[50%] p-2 px-4 z-10 h-[15%] min-h-[85px] sm:h-[25%] md:h-[20%] lg:h-[25%] xl:min-h-[90px] xl:w-[85%] xl:h-[20%] xl:max-h-[96px] `} >
                    <h1 className="font-bold text-black "> Holy Quran </h1>
                    <div className="flex w-full items-center justify-center gap-5 mt-3 text-sm xl:text-base ">
                    <div className="flex items-center gap-2 hover:text-black duration-300 hover:font-semibold ">
                    <h1>Code</h1>
                    <AiFillGithub className=" text-xl xl:text-2xl"></AiFillGithub>
                </div>
                <div className="flex items-center gap-2 hover:text-black duration-300 hover:font-semibold ">
                    <h1>Live Demo</h1>
                    <RiShareBoxLine   className=" text-2xl xl:text-3xl"></RiShareBoxLine>
                </div>
                    </div>
            </div>
            </>
